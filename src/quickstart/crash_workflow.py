...
def fulfill_slowly(order: dict) -> dict:
    ...
    before Aga records the step result. The final shipment effect uses a stable
    key in a real provider. This audit file intentionally makes the uncertainty
    observable for the tutorial.
    """
    path = Path(os.getenv("AGA_CRASH_AUDIT", "audit.log"))
    with path.open("a", encoding="utf-8") as handle:
        handle.write(f"START {order['id']} pid={os.getpid()}\n")
        handle.flush()
    time.sleep(float(os.getenv("AGA_CRASH_STEP_SECONDS", "8")))
    with path.open("a", encoding="utf-8") as handle:
