...
| [One application, five workflows](docs/primitives.md#see-one-application-across-five-workflows) | `primitives.loan_journey` | five concurrent roots with four Steps each, one four-Step document child, and one exact-resource Case layout; simulated work only |
...
| [Workflow-owned agent pattern](docs/agent-pattern.md) | `agent_pattern` | committed model decisions, parallel tools, approval/effect boundaries, and a long logical session split across bounded Run generations |
| [Command mailbox and fleet registry](docs/agent-pattern.md#drive-a-long-lived-session-with-commands) | `agent_pattern.command_session` | ordered authenticated commands, worker restart, bounded continuation, and desired-versus-observed fleet state |
